...
| `NETVIZ_HOME_IPS` | empty | This site's own WAN address(es)/CIDRs, comma separated, so inbound blocks are not attributed to your own ISP's country. Keep it current with `tools/refresh_home_ip.sh` |
...
| `NETVIZ_LOG_MISSES` | `0` | Log this many unplaceable source addresses, for naming a miss burst |
