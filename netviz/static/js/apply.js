...
export const HANDLERS = {
  ...
  // createMenu reads cfg('test.preview.*') fresh on every hover, so nothing
  // here has a live object to poke -- the same shape traffic.*'s settings have.
  ...Object.fromEntries(paths().filter((p) => p.startsWith('test.'))
    .map((p) => [p, configOnly])),
