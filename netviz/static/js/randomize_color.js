// The randomizer.
//
// It produces valid values BY CONSTRUCTION, never by rolling and hoping. The
 ...
 *  and a random roll is meant to be surprising rather than absent. */
...
/** Every element rolled independently, ignoring the ramp entirely.
 *
 ...
 *  Rolling it would make this a button that half the time reports an error. */
export function randomizeColors(rand = Math.random) {
 ...
 *  The roll reaches arc colors but never the sky, so the sky is the fixed side and
...
export const ATMOSPHERE_RANDOM = {
...
/** Every schema path the randomizer writes. The theme panel's snapshot is
 *  built from this, so Revert and Close cover the whole roll -- a path it can write
 *  and Revert cannot restore is a one-way door. */
export const RANDOMIZE_PATHS = [
  ...
  .concat(Object.keys(ATMOSPHERE_RANDOM));

/** The whole display rolled independently, ignoring the ramp entirely.
 *
 *  The sky is NOT in the output. It is the one value that decides whether
 *  anything else is legible and the one path that refuses rather than clamps.
 *  Rolling it would make this a button that half the time reports an error --
...
export function randomizePatch(rand = Math.random, opts = {}) {
  const {
  ...
  const out = {};
  const colors = randomizeColors(rand);
